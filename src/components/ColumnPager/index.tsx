...
import { groupBy, isEmpty } from './utils/collection';
...
const ColumnPager = ({
            ...
            {Array.from({ length: pageColumnCount }).map((_, colIndex) => (
              <Column key={String(colIndex)} className={columnClassName}>
                {columnData?.[colIndex]?.map((item) => {
                  ...
                  const translateX = (item.croppedItemIndex ?? 0) * (item.innerItemWidth ?? 0);
                  const translateY = item.croppedItemIndex === 0 ? (item.offsetHeight ?? 0) : 0;
  ...
  useEffect(() => {
    ...
    const checkAndGenerate = () => {
      ...
      // stable=true 발견 후 한 사이클 더 wait — Sentence의 useMeasureWords 이후
      // decorator 내부(예: Memo의 clampedLeft useLayoutEffect)에서 추가 레이아웃
      // 보정이 발생할 수 있어 outerHTML 추출 전에 그 사이클까지 settle 보장
