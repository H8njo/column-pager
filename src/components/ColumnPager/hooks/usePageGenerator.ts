...
const getPageHeight = async (
  ...
  if (!pageLayoutCache.has(pageNumber)) {
    ...
    if (pageColumnSize) {
      pageLayoutCache.set(pageNumber, pageColumnSize);
    }
  }

  return pageLayoutCache.get(pageNumber)?.height ?? 0;
};
  ...
  for (let i = 0; i < measuredItems.length; i++) {
    ...
    if (itemHeight > containerHeight) {
      const measuredOverflow = await measureElementSize({
        ...
        offsetHeight: columnHeightAccumulated,
      ...
      const cropCount = Math.floor((innerDivSize?.width ?? 0) / (innerSize?.width ?? 1));

      const firstItemHeight = containerHeight - columnHeightAccumulated;
      ...
      for (let currentCropIndex = 0; currentCropIndex < cropCount; currentCropIndex++) {
        ...
        const croppedItem: ItemLayout = {
          ...
          // Crop 정보
          height,
          offsetHeight: croppedItemIndex > 0 ? columnHeightAccumulated : 0,
