...
- **반응형 폭 (A4 고정 제거)**: ColumnPager는 멀티컬럼 레이아웃 렌더러로, A4/PDF 고정 크기는 V1 잔재라 제거. 폭은 컨테이너에서 측정(반응형), 높이만 `pageHeight` prop. 리사이즈는 ResizeObserver + 디바운스(`resizeDebounceMs`, 기본 150) 후 재페이지네이션. (`A4`→`PageSheet`, `pageSize`/`orientation`/프리셋 폐기)
- **렌더 결과만 방출 (문서 변환은 소비자)**: `onPagesGenerated(pages, html)`의 `html`은 렌더된 컨테이너 `outerHTML`. 폰트 `@import`·`@page`·스타일시트 인라인 등 PDF/문서 변환은 소비자 관심사라 라이브러리에서 제거(`convertElementToHtmlString`/`htmlOptions` 폐기).
- **공개 유틸**: `isSliced` / `getPageCount` / `findItemPage` / `getSectionPageRanges`.
- **가드/통지**: `columnHeight<=0` 가드(측정 실패 시 퇴화 방지), `onError` 콜백.
- **prop 변경 감지**: `blocksSignature`가 props(children·함수 제외) 직렬화 포함 → 콘텐츠를 prop으로 받는 카드(`<Card lines={...}/>`)도 변경 시 재페이지네이션 + 캐시 무효화.
- **per-item 측정 캐시**: `(콘텐츠 시그니처 + columnWidth)` 키. 바뀐 카드만 재측정(나머지 캐시 히트). 무거운 카드도 변경분만 측정.
- **지연 최소화 (정확도 유지)**: 측정은 `flushSync` 동기 커밋 + `getBoundingClientRect` 강제 레이아웃으로 **rAF 대기 제거**(폰트 의존성만 `waitForFonts` 유지). emit은 StableGate 없으면 **즉시 emit**(워밍업/settle 프레임 생략). 무거운 카드: ItemCell/SliceView `memo`로 node ref 안정 시 재렌더 생략(소비자 메모이즈와 결합).
- **options 변경 재페이지네이션**: `moveOversizedItemToNextColumn` 등 `options`도 직렬화 키로 재계산 트리거에 포함(매 렌더 새 객체로 와도 내용 동일하면 안정). 값이 바뀌어야만 재계산.
- **더블 페인트 주의 (스크롤바)**: 폭을 컨테이너에서 측정하므로, 첫 페인트로 내용이 길어져 **세로 스크롤바가 뒤늦게 생기면** 가용 폭이 줄어 2차 페이지네이션이 일어난다(슬라이스 `translateX`가 컬럼 폭에 비례 → 눈에 띄게 점프). 스크롤 컨테이너에 `scrollbar-gutter: stable`(또는 `overflow-y: scroll`)로 거터를 미리 예약하면 폭이 안정되어 사라진다. 스토리북은 `.storybook/preview-head.html`에서 적용.
- **테스트**: core 유닛 + usePagination 훅 유닛 = 49개.
